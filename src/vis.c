...
/***
   ...
   *
    $ mpicc -o mpe_pi_X11 mpe_pi_X11.c      \
...
/* declare MPE graphics "object" */
   /* ....Fill in, please.... */
    ...
    if (myid == 0) { /* read epsilon from command line */
        if( argv[1] != NULL ) sscanf( argv[1], "%lf", &epsilon );        
    }
...
/* open MPE graphics window */
   /* ....Fill in, please.... */
    MPE_Open_graphics(&handle, world, NULL, -1, -1, WINDOW_SIZE, WINDOW_SIZE, 1);
...
/* draw point in MPE graphics window */
   /* ....Fill in, please.... */
        MPE_Draw_point(handle, (x * (WINDOW_SIZE/4) + WINDOW_SIZE/2), (y * (WINDOW_SIZE/4) + WINDOW_SIZE/2), MPE_RED);
		}
		else
		    out++;
	    }
        ...
        for(int i = 0; i < 90;i++){
                for(int j = 0; j< 60;j++){
                    MPE_Color clr = MPE_WHITE;
                    if(j > 30){
                        clr = MPE_RED;
                    }

                    MPE_Draw_point(handle, i,j , clr);
                }
            }
        MPE_Draw_line(handle, 0, 0, 90, 0, MPE_BLACK);
        MPE_Draw_line(handle, 90, 0, 90, 60, MPE_BLACK);
        MPE_Draw_line(handle, 90, 0, 90, 0, MPE_BLACK);
        MPE_Draw_line(handle, 0, 60, 90, 60, MPE_BLACK);
